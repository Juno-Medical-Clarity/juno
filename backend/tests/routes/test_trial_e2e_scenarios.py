"""End-to-end / integration tests for the trial pathway (POST /trial/jobs ->
enqueue -> worker -> completed/failed doc -> DELETE /trial/jobs/<id>).

Scope (per .dev/trial-simplify/scenario-validation-2026-09-05.md): drive the
REAL Flask routes (routes/trial.py, routes/worker.py) and the REAL helper
...
mocked:
  ...
  - Cloud Tasks:     routes.trial.enqueue_job_safe mocked (no real dispatch);
                     the worker route is invoked directly and synchronously
                     in its place, exactly as Cloud Tasks would call it.
  - LLM/Vertex:      routes.worker.PIPELINES is overridden with a fake
                     generator that yields real Pydantic CarePlanV1_2/Grading
                     ...
                     (via CarePlanV1_2Pipeline's own _generate_text/
                     _generate_json wrappers) are mocked, so the REAL
                     exception classification path (care_plan/v1_2/
...
WORKER_HEADERS = {"X-CloudTasks-QueueName": "trial-queue"}
...
def fake_db(monkeypatch):
    ...
    monkeypatch.setattr("routes.trial.firestore_client", lambda: client)
    return client
...
def _post_job(client_trial, headers, mock_gcs=True, **kwargs):
    with patch.dict("os.environ", TRIAL_ENV):
        with patch("routes.trial.enqueue_job_safe", return_value=None):
            if mock_gcs:
                with patch("services.care_plan_input.get_gcs_bucket") as mock_bucket:
                    ...
                    resp = client_trial.post("/trial/jobs", headers=headers, **kwargs)
            else:
                resp = client_trial.post("/trial/jobs", headers=headers, **kwargs)
    return resp
...
def _delete_job(client_trial, job_id, headers):
    with patch("utils.gcs.delete_gcs_object") as mock_delete:
        resp = client_trial.delete(f"/trial/jobs/{job_id}", headers=headers)
...
def _build_care_plan(size: str = "typical"):
    """Build a real CarePlanV1_2 instance with realistic field lengths, so
    envelope.to_dict() serialization in the worker is genuine, not mocked."""
    from models.care_plan.versions.v1_2 import (
        CarePlanV1_2, ReasonForVisit, Diagnosis, DiagnosisDetail, Medication,
...
class TestScenario1TypicalDischargeSummary:
    def test_full_chain_completes_and_final_doc_is_safe_and_small(self, client_trial, client_worker, fake_db, auth_anon):
        ...
        resp = _post_job(client_trial, auth_anon, json={"text": text})
        assert resp.status_code == 202
        job_id = resp.get_json()["job_id"]

        with patch.dict("routes.worker.PIPELINES", {"v1-2": fake_pipeline}):
        ...
        # Only "combined" grading entries survive for trial docs.
        ...
        del_resp, mock_delete = _delete_job(client_trial, job_id, auth_anon)
...
class TestScenario2PhonePhotosOCR:
    def test_three_jpegs_via_ocr_completes(self, client_trial, client_worker, fake_db, auth_anon):
        ...
        with patch("services.care_plan_input.extract_text_from_image", side_effect=ocr_texts):
            resp = _post_job(
                client_trial, auth_anon, data=data, content_type="multipart/form-data",
            )
        assert resp.status_code == 202
        job_id = resp.get_json()["job_id"]
        doc = fake_db.raw_doc("care_plan_outputs", job_id)
...
class TestScenario3MixedBatchLimits:
    def test_five_mixed_files_just_under_10mb_succeeds(self, client_trial, fake_db, auth_anon):
        from pypdf import PdfWriter
        ...
        with patch("services.care_plan_input.extract_text_from_pdf", return_value="PDF page content about the visit."):
            ...
            with patch("services.care_plan_input.extract_text_from_image", return_value="Photo OCR text about the visit."):
                resp = _post_job(client_trial, auth_anon, data=data, content_type="multipart/form-data")
    ...
    def test_five_files_just_over_10mb_returns_clean_400(self, client_trial, auth_anon):
        over_budget = Constants.Trial.MAX_AGGREGATE_FILE_BYTES + 1024
        data = {"files": [
    ...
    def test_six_files_returns_clean_400(self, client_trial, auth_anon):
...
class TestScenario4MultibyteNearLimits:
    def test_cjk_under_char_cap_over_byte_cap_returns_clean_400(self, client_trial, auth_anon):
    ...
    def test_arabic_rtl_over_byte_cap_returns_clean_400(self, client_trial, auth_anon):
    ...
    def test_emoji_surrogate_pairs_over_byte_cap_returns_clean_400(self, client_trial, auth_anon):
    ...
    def test_mixed_multibyte_just_under_both_caps_completes_and_stays_under_1mib(
        self, client_trial, client_worker, fake_db, auth_anon,
    ):
    ...
    def test_combining_diacritics_and_null_and_lone_surrogate_rejected_cleanly(self, client_trial, fake_db, auth_anon):
        # NUL byte.
        resp = _post_job(client_trial, auth_anon, json={"text": "Patient note\x00with a null byte"})
        assert resp.status_code == 400
        _assert_no_internal_leak(resp)

        # Lone UTF-16 surrogate via raw JSON body (json.loads permits this;
        # UTF-8 encoding does not).
        resp2 = client_trial.post(
            "/trial/jobs",
        ...
        resp3 = _post_job(client_trial, auth_anon, json={"text": combining})
...
class TestScenario5DegenerateInputs:
    def test_scanned_pdf_no_text_layer_returns_clean_error(self, client_trial, auth_anon):
    ...
    def test_zero_byte_file_returns_clean_error(self, client_trial, auth_anon):
        data = {"files": (io.BytesIO(b""), "empty.pdf")}
        resp = _post_job(client_trial, auth_anon, data=data, content_type="multipart/form-data")
        _assert_no_internal_leak(resp)
        assert resp.status_code // 100 == 4

    def test_corrupt_pdf_returns_clean_error(self, client_trial, auth_anon):
    ...
    def test_password_protected_pdf_returns_clean_error(self, client_trial, auth_anon):
    ...
    def test_zip_disguised_as_pdf_returns_clean_error(self, client_trial, auth_anon):
    ...
    def test_blank_docx_returns_clean_error(self, client_trial, auth_anon):
    ...
    def test_whitespace_only_pasted_text_treated_as_no_input(self, client_trial, auth_anon):
        resp = _post_job(client_trial, auth_anon, json={"text": "   \n\t  "})
        _assert_no_internal_leak(resp)
        assert resp.status_code == 400

    def test_null_byte_in_pasted_text_returns_clean_error(self, client_trial, auth_anon):
        resp = _post_job(client_trial, auth_anon, json={"text": "clinical note\x00with embedded null"})
        _assert_no_internal_leak(resp)
        assert resp.status_code == 400
        assert resp.get_json()["error"]["code"] == "INPUT_VALIDATION_ERROR"

    def test_lone_surrogate_in_pasted_text_returns_clean_error(self, client_trial, auth_anon):
...
class TestScenario6PartialFailureBatch:
    def test_two_of_five_unusable_succeeds_with_skipped_files_reported(self, client_trial, fake_db, auth_anon):
    ...
    def test_all_five_unusable_returns_single_clean_empty_document_error(self, client_trial, auth_anon):
        data = {"files": [
...
class TestScenario7FilenameHostility:
    def test_unicode_emoji_filename_accepted_and_isolated_from_gcs_path(self, client_trial, fake_db, auth_anon):
        ...
        with patch("services.care_plan_input.get_gcs_bucket") as mock_bucket:
            ...
            with patch.dict("os.environ", TRIAL_ENV):
                with patch("routes.trial.enqueue_job_safe", return_value=None):
                    resp = client_trial.post(
                        "/trial/jobs", data=data, content_type="multipart/form-data", headers=auth_anon,
                    )
            assert resp.status_code == 202
            ...
            assert called_path.startswith("care_plan_trial/anon-1/inputs/")
            assert called_path.endswith(".pdf")

    def test_255_char_filename_accepted(self, client_trial, fake_db, auth_anon):
    ...
    def test_path_traversal_filename_does_not_escape_gcs_path(self, client_trial, fake_db, auth_anon):
    ...
    def test_filename_with_newline_and_quote_does_not_break_storage_or_leak(self):
        ...
        resolved, _ = resolve_uploaded_files([upload])
...
class TestScenario8LifecycleRaces:
    def test_delete_mid_worker_write_does_not_resurrect_doc_and_gcs_not_orphaned(
        ...
        fake_db.collection("care_plan_outputs").document(job_id).set({
            ...
            "input_pdf_gcs_uri": "gs://test-bucket/care_plan_trial/anon-1/inputs/abc.pdf",
            "input_version": "v1-2", "grading_enabled": False,
            "is_trial": True, "expires_at": now + timedelta(hours=1),
        })
        ...
        with patch("utils.gcs.delete_gcs_object") as mock_delete_gcs:
            with patch.dict("routes.worker.PIPELINES", {"v1-2": racing_pipeline}):
                worker_resp = _run_worker(client_worker, job_id)
        ...
        mock_delete_gcs.assert_called_once_with(
            "gs://test-bucket/care_plan_trial/anon-1/inputs/abc.pdf"
        )
...
class TestScenario9FailureSurfaces:
    ...
    def test_llm_429_quota_exceeded_surfaces_as_safe_terminal_error(self, client_worker, fake_db):
        ...
        with patch("care_plan.v1_2.pipeline.CarePlanV1_2Pipeline.__init__", return_value=None):
            with patch(
                "care_plan.v1_2.pipeline.CarePlanV1_2Pipeline._generate_text",
        ...
        assert "input_text" not in doc  # cleared on every terminal state for trial jobs
    ...
    def test_llm_malformed_json_surfaces_as_safe_terminal_error(self, client_worker, fake_db):
        ...
        with patch("care_plan.v1_2.pipeline.CarePlanV1_2Pipeline.__init__", return_value=None):
            with patch(
                ...
                with patch(
                    "care_plan.v1_2.pipeline.CarePlanV1_2Pipeline._generate_json",
                    side_effect=JunoError(ErrorCode.LLM_INVALID_JSON, detail="not json"),
                ):
                    resp = _run_worker(client_worker, job_id)
    ...
    def test_worker_hard_kill_leaves_job_stuck_processing_until_client_watchdog(self, client_worker, fake_db):
        ...
        job) and retries, or (b) frontend-trial/src/components/
...
class TestScenario10RateLimiting:
    ...
    def test_burst_within_limit_then_blocked_real_check_rate_limit(self, client_trial, monkeypatch):
        ...
        limit = Constants.Trial.RATE_LIMIT_PER_IP_PER_HOUR
        statuses = []
        for _ in range(limit + 2):
            resp = client_trial.post(
                "/trial/jobs", data=json.dumps({"text": "hi"}), headers=headers_json,
    ...
    def test_trusted_proxy_hop_selection_cannot_be_bypassed_by_spoofed_xff(self, client_trial, monkeypatch):
        from utils.rate_limit import get_client_ip
        app = client_trial.application
    ...
    def test_rate_limiter_firestore_failure_fails_open_not_500(self, client_trial, monkeypatch):
        def _boom():
            raise RuntimeError("Firestore unavailable")

        monkeypatch.setattr("utils.rate_limit.check_rate_limit", _boom)
        with patch.dict("os.environ", TRIAL_ENV):
            with patch("routes.trial.create_job_doc"):
                with patch("routes.trial.enqueue_job_safe", return_value=None):
                    resp = client_trial.post(
                        "/trial/jobs", json={"text": "hi"},
...
class TestScenario11AccessControl:
    def test_anon_user_b_cannot_delete_anon_user_as_job(self, client_trial, fake_db, monkeypatch):
        job_id = str(uuid.uuid4())
        now = datetime.now(timezone.utc)
        fake_db.collection("care_plan_outputs").document(job_id).set({
        ...
        resp, mock_delete = _delete_job(client_trial, job_id, headers_b)
        assert resp.status_code == 403
        mock_delete.assert_not_called()
        # Doc must survive untouched.
        assert fake_db.raw_doc("care_plan_outputs", job_id) is not None

    def test_anon_user_a_can_delete_their_own_job(self, client_trial, fake_db, monkeypatch):
        job_id = str(uuid.uuid4())
        now = datetime.now(timezone.utc)
        fake_db.collection("care_plan_outputs").document(job_id).set({
            "uid": "anon-A", "name": "Test", "source_filename": "text_input",
        ...
        resp, _ = _delete_job(client_trial, job_id, headers_a)
    ...
    def test_firestore_rules_static_review_note(self):
        ...
        (JobDoc.for_single) and no trial code path ever flips it, an
