...
// A UTF-16 code unit (what JS string `.length` counts) encodes to at most 3 UTF-8
// bytes -- a non-ASCII BMP character. (A surrogate pair -- 2 code units -- encodes
// to 4 bytes, i.e. only 2 bytes/unit, so 3 is the true per-unit ceiling.) So the
// byte cap can only be exceeded once the character count passes MAX_TEXT_BYTES / 3;
// below that, byte length is guaranteed to be under the cap and we can skip the
// (comparatively expensive) UTF-8 encode. This keeps validateText() cheap for the
// common case of running on every keystroke/paste in a large textarea.
