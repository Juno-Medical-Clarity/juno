...
// How long we're willing to sit in 'pending' before giving up and surfacing
// the Retry button (edge-case review #2). If signInAnonymously's promise
// never settles, or settles without onAuthStateChanged ever firing with a
// non-null user, authState would otherwise stay 'pending' forever — the
// Retry button only renders in the 'error' branch, so a hung pending state
// left the user with no actionable UI at all. Mirrors the bound
// trialApi.ts::waitForAuthUser already applies to this exact race.
