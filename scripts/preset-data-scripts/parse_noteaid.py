...
This file combines the online data-generation script and the offline ingestion
parser for the NoteAid-README dataset. Both do essentially the same thing —
load a CSV variant from HuggingFace and write per-sample files — but with
slightly different CLI interfaces that are unified here.
...
Usage:
    ...
    /root/projects/juno/.venv/bin/python scripts/preset-data-scripts/parse_noteaid.py
    ...
    # Legacy alias: --dest is accepted in place of --output-dir
    python parse_noteaid.py --dest /custom/path/preset-data
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    # Legacy alias: --dest (from the backend parser variant)
    parser.add_argument(
        "--dest",
        type=Path,
        default=None,
        dest="dest",
        help="Alias for --output-dir (legacy compatibility)",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    # Resolve output directory: --output-dir takes precedence over --dest
    if args.output_dir is not None:
        args.resolved_output_dir = args.output_dir
    elif args.dest is not None:
        args.resolved_output_dir = args.dest
    else:
        args.resolved_output_dir = DEFAULT_OUTPUT_DIR
...
def main() -> None:
    ...
    dest_root = args.resolved_output_dir / GROUP_NAME
