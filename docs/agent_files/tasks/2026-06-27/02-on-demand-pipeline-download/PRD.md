...
- `[RESOLVED: use INTERNAL_ERROR as top-level error code; add DATASET_DOWNLOAD_ERROR as a sub-error/inner error using the existing sub-error pattern in error_codes.py]` Error code for GCS download failure in the worker: the existing `PipelineErrorCode` enum in `error_codes.py` does not have a `DATASET_DOWNLOAD_ERROR` value. Determine whether to add one or reuse `INTERNAL_ERROR` for this failure mode.
