...
| Q5 | `utils/env.py`'s `get_env()` has zero production call sites today — is it dead code that should be deleted instead of merged forward? | [RESOLVED: keep get_env() despite zero current call sites — it is a deliberate future test-mocking patch point per its docstring; merge it into utils/misc.py as already shown in §4c rather than deleting it] |
